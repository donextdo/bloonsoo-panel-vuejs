...
const updateRate = async (rate) => {
    try {
        const data = await axios.post(`/api/commission/update`,
            {
                rate
            },
            {
...
<template>
    <Card :title="`Commission Rate ${rate}%`">
        ...
        <button 
        @click="togglePopup"
        class="absolute right-6 top-10 text-2xl text-blue-600 hover:text-gray-700">
        ...
        <div 
        v-if="!loading && commisionData.length == 0"
        class="w-full h-96 flex items-center justify-center flex-col">

            <font-awesome-icon icon="fa-solid fa-coins" class="text-6xl text-gray-400"/>
            <p class="mt-4 text-base text-gray-500 font-light">No commission recoreds</p>
